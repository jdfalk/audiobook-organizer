...
func TestEventHub_HandleSSE(t *testing.T) {
	gin.SetMode(gin.TestMode)
	
	hub := NewEventHub()
	router := gin.New()
	router.GET("/sse", hub.HandleSSE)
	...
	// Check that we got at least the initial connection event
	body := w.Body.String()
...
func TestEventHub_HandleSSE_WithOperationSubscription(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	// Create a test request with operation query parameter
	req, err := http.NewRequest("GET", "/sse?operation=op123", nil)
	if err != nil {
		t.Fatalf("Failed to create request: %v", err)
	}
	...
	hub.SendOperationProgress("op123", 50, 100, "Testing")
	
	time.Sleep(100 * time.Millisecond)
	
	body := w.Body.String()
	if !strings.Contains(body, "op123") {
		t.Error("Expected operation event for op123")
	}
}
