...
// for AES-256. A random 16-byte salt is generated by the caller and stored
// alongside the derived key so the derivation is reproducible.
//
...
//
// A fresh random salt is generated on every call. The derived key and its
// hex-encoded salt are returned so callers can persist the salt alongside
// any encrypted data for future decryption via DeriveKeyFromPasswordWithSalt.
//
...
func DeriveKeyFromPassword(password string) []byte {
	// Generate a random salt. On failure we fall back to a zero salt, which
	// degrades security but keeps the function callable (callers that ignore
	// the salt will still get a deterministic key for the empty-salt path).
	salt := make([]byte, argon2idSaltLen)
	if _, err := io.ReadFull(rand.Reader, salt); err != nil {
		salt = make([]byte, argon2idSaltLen) // zero salt on RNG failure
	}
