...
class TestDashboardWorkflow:
    ...
    def test_dashboard_loads(self, authenticated_driver):
        ...
        assert (
            "Dashboard" in driver.page_source
            or "Library Statistics" in driver.page_source
        )
    ...
    def test_import_folders_display(self, authenticated_driver):
        ...
        assert "import" in page_source or "folder" in page_source, (
            "Dashboard missing folder-related content"
        )
        ...
        try:
            ...
            folder_elements = driver.find_elements(
                By.XPATH,
                "//*[contains(text(), 'Import Folders') or contains(text(), 'import folders')]",
            )
            assert len(folder_elements) > 0, (
                "No 'Import Folders' text found on dashboard"
            )
