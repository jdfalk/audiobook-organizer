...
- [~] **4.16** `/parallel-sweep` slash command (**L**) — successor to `parallel-refactor-sweep`. Project-scope command + skill that drives the full sweep autonomously: worktree isolation hooks, parallel child dispatch, PR + admin-merge with local-`make ci` gate, sibling-rebase loop with Sonnet/Opus conflict resolvers, resume across usage limits via persisted state file. Plan: [`docs/superpowers/plans/2026-04-24-parallel-sweep-slash-command.md`](docs/superpowers/plans/2026-04-24-parallel-sweep-slash-command.md) (9-step build). Steps 1-8 **complete** Apr 24-25 2026 (skeleton + state, coordinator + child prompts, dispatch + hook spike, PR + merge pipeline, sibling-rebase clean case, Sonnet conflict resolver, Opus file-copy fallback, resume support). Fallback uses per-commit cherry-pick (rebase/FF-only; no squash). Resume uses `git reset --hard origin/main` for in-flight tasks. Two empirical spikes confirmed: hook does NOT fire for sub-agents and Sonnet resolves trivial conflicts cleanly. Step 9 (polish) is the last step. Future work: extract universal version to `~/.claude/commands/` after ~3 real sweeps.
