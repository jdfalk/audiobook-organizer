...
- [ ] **4.17** Audit divergent fetch-path code and unify behind shared helpers (**M**) — `bulkFetchMetadata` in `metadata_handlers.go` re-implements the source-chain loop, scoring, and cache logic that `metafetch.Service.FetchMetadataForBook` already owns. Also: TTL-check snippets are copy-pasted in `bulkFetchMetadata` and `service.go`. Goal: single `FetchMetadataForBook(ctx, bookID, opts)` call path so all cache/retry/scoring behaviour is DRY; handlers become thin orchestrators. Audit all other handler/service pairs for similar divergence before implementing.
...
- [x] **4.16** `/parallel-sweep` slash command (**L**) — **complete** (9-step build, Apr 24-25 2026). Project-scope command + skill that drives the full sweep autonomously: worktree isolation, parallel child dispatch, PR + admin-merge with local-`make ci` gate, sibling-rebase loop with Sonnet/Opus conflict resolvers, resume across usage limits via persisted state file. Per-commit cherry-pick fallback (rebase/FF-only; no squash). 87/87 unit tests green. Two empirical spikes confirmed: PreToolUse hook does NOT fire for sub-agents (post-hoc `git status` is the load-bearing isolation barrier); Sonnet resolves trivial conflicts cleanly given the prompt. Plan: [`docs/superpowers/plans/2026-04-24-parallel-sweep-slash-command.md`](docs/superpowers/plans/2026-04-24-parallel-sweep-slash-command.md). User spec: [`docs/superpowers/specs/parallel-sweep.md`](docs/superpowers/specs/parallel-sweep.md). Future work: extract universal version to `~/.claude/commands/` after ~3 real sweeps. Future work: extract universal version to `~/.claude/commands/` after ~3 real sweeps.
...
> - **4.17** — Refactor `bulkFetchMetadata` (server/metadata_handlers.go) to delegate to `metafetch.Service.FetchMetadataForBook` per book — the handler duplicates the source-loop logic from the service and diverged (no cache, no ContextualSearch, simpler scoring, no retry variants). Centralizing removes two divergent copies of "try sources, score results, apply best". Also audit all handler/service pairs for similar TTL-check or source-chain duplication. Medium effort.
...
- [ ] **AI-BATCH-2** Migrate `OpenAIParser.ParseCoverArt` (openai_parser.go:364) and its caller in `resolveProductionAuthor` (entities_handlers.go:815) to aijobs. Requires decoupling result application from operation loop: currently ParseCoverArt is called synchronously within the background operation's book-processing loop, with results immediately applied to DB (create/update authors, set book associations). Async design: extract cover art extraction into operation, persist image bytes with tempfile ID, submit batch with image IDs, register callback to apply author lookups/updates once results arrive. Deferred to follow-up PR due to operation architecture scope. Task 2.3 deferred as out-of-scope; marked in allow-list with this TODO.
...
- [x] [2026-04-23 Envelope migration (parallel) (4.15)](docs/superpowers/plans/2026-04-23-envelope-migration-parallel.md) — complete, all 24 handlers migrated (PRs #425–#438)
