...
### 5.9 Enforce mockery-generated mocks (**S**) — ✅ DONE

**Shipped:** This PR (refactor/mockery-conversion, post-#241).

**What shipped:**

1. **11 hand-written test doubles converted to mockery-generated
   mocks** across cmd, logger, ai, and metadata. New `.mockery.yaml`
   entries for `ProgressReporter`, `MetadataSource`, `ContextualSearch`,
   a second in-package `MetadataExtractor`, `MetadataCandidateScorer`,
   `metadataLLMBackend` (private, in-package `_test.go`),
   `OperationStore`, `RealtimeHub`, `ActivityLogWriter`, and
   `RetentionStore`.
2. **`make vet`** target added and wired into `make test` — runs
   `go vet ./...` before tests. The stubStore / PR #234 / PR #241
   drift incident would have been caught here.
3. **`make mocks`** target — regenerate all mockery-managed mocks.
4. **`make mocks-check`** target — regenerate then `git diff` to
   fail if committed mocks are stale. Wired into `make ci`.
5. **Compile-time assertion** `var _ Store = (*MockStore)(nil)` on
   the hand-written `internal/database/mock_store.go`, with a doc
   comment explaining why it's intentionally kept (see below).

**What was NOT converted — intentional:**

The hand-written `internal/database/mock_store.go` (a 1578-line
functional-option mock used by ~22 server test files) is kept as-is.
Converting it to mockery would either:
- Force ~22 test files to enumerate `.Maybe()` expectations for
  every Store method each code path happens to touch (tedious and
  noisy), or
- Require a loose-mock helper that re-implements this file's
  permissive behavior on top of mockery's strict-expectations model
  (which defeats the conversion).

Since this mock is heavily used, `go vet ./...` already catches drift
(unlike the `stubStore` incident, which happened in an under-exercised
file). The compile-time assertion makes that safety net explicit, and
the new `make vet` target in CI enforces it.

A future follow-up could write a custom mockery template that
generates the functional-option pattern directly from the Store
interface, replacing the hand-written file. That's a maybe.

**Two tag.Metadata mocks** in `internal/mediainfo/mediainfo_test.go`
and `internal/metadata/write_roundtrip_test.go` are also kept: they
implement an external interface from the cgo-bound `tag` package,
which would complicate mockery config for marginal benefit.
