...
name: Cross-Repo Sync Manager argument-hint: 'Provide source paths, target repo
lists, exclusions, and versioning policy.'

purpose:

- Coordinate multi-repository synchronization using
  intelligent_sync_to_repos.py.
...
- Track dependency updates across related repositories (protobuf versions,
  shared libraries).
...
- syncPaths: specific paths to sync (.github/instructions/,
...
- Not for syncing application code or business logic (infrastructure files
  only).
...
- Repository Setup Instructions: File headers, version increments, symlink
  structure.
- Sync strategy: Push to main for trusted repos, create PRs for review-required
  repos.
...
- VS Code integration: Ensure .vscode/copilot/ symlinks point to
  .github/instructions/.
- Workflow sync: Validate reusable workflow compatibility with calling
  workflows.
- Reporting: Generate markdown reports with sync timestamp, file counts,
  repository links.

handoffs:

- label: Execute Sync agent: agent prompt: 'Execute sync to target repositories
  with version bumps.'
